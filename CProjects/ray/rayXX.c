...
triple reflect(triple initial)
{
   double a = 2 * dotp(initial, normal);
   triple reflection = {initial.x - a * normal.x, initial.y - a * normal.y, initial.z - a * normal.z};
   return reflection;
}
...
color castRay(triple point, triple initial, color hitColor, int depth)
{
   if(depth > MAXDEPTH) 
   {
      return sky;
   }
   triple reflection = reflect(initial);
   color addition;
   double Tf = checkSphere(point, reflection, a[0].c, a[0].r);
   double Tr = checkSphere(point, reflection, a[1].c, a[1].r);
   double Tg = checkSphere(point, reflection, a[2].c, a[2].r);
   double Tb = checkSphere(point, reflection, a[3].c, a[3].r);
   double Tmine = min(min(Tf, Tr), min(Tg, Tb));
   if(Tmine == MAX)
   {
      hitColor.r = hitColor.r * 0.2 + 0.8 * sky.r;
      hitColor.g = hitColor.g * 0.2 + 0.8 * sky.r;
      hitColor.b = hitColor.b * 0.2 + 0.8 * sky.r;
      return hitColor;
   }
   double spherex = point.x + Tmine * reflection.x;
   double spherey = point.y + Tmine * reflection.y;
   double spherez = point.z + Tmine * reflection.z;
   if(Tmine == Tf)
   {
      printf("WAEOFUHAWEFJAWOEFIJAOPWIEJFPAOWIEJDFOPAWJIEFOPAJDFPOAHEFAOWUFE");
      addition = a[0].h;
      // addition = castRay(point, reflection, (triple) {(spherex - a[0].c.x)/a[0].r, (spherey - a[0].c.y)/a[0].r, (spherez - a[0].c.z)/a[0].r}, a[0].h, depth + 1);
   }
   else if(Tmine == Tr)
   {
      addition = a[1].h;
      // addition = castRay(point, reflection, (triple) {(spherex - a[1].c.x)/a[1].r, (spherey - a[1].c.y)/a[1].r, (spherez - a[1].c.z)/a[1].r}, a[1].h, depth + 1);
   }
   else if(Tmine == Tg)
   {
      addition = a[2].h;
      // addition = castRay(point, reflection, (triple) {(spherex - a[2].c.x)/a[2].r, (spherey - a[2].c.y)/a[2].r, (spherez - a[2].c.z)/a[2].r}, a[2].h, depth + 1);
   }
   else if(Tmine == Tb)
   {
      addition = a[3].h;
      // addition = castRay(point, reflection, (triple) {(spherex - a[3].c.x)/a[3].r, (spherey - a[3].c.y)/a[3].r, (spherez - a[3].c.z)/a[3].r}, a[3].h, depth + 1);
   }
   hitColor.r = hitColor.r * 0.2 + 0.8 * addition.r;
   hitColor.g = hitColor.g * 0.2 + 0.8 * addition.g;
   hitColor.b = hitColor.b * 0.2 + 0.8 * addition.b;
   return hitColor;
}
...
int main(void)
{
   ...
   for( y = 0 ; y < N ; y++ )
   {
      for( x = 0 ; x < M ; x++)
      {
         ...
         color pointColor = findColor(x, y, curr, e, sphere_points);
         color reflectColor;
         if(pointColor.r == sky.r && pointColor.g == sky.g && pointColor.b == sky.b)
         {
            reflectColor = pointColor;
         }
         else
         {
            reflectColor = castRay(*sphere_points[y][x], curr, pointColor, 0);
         }
      ...
      //             else
      //             {
      //                rgb[y][x][0] = purple.r/2   ; // red
      //                rgb[y][x][1] = purple.g/2   ; // green
      //                rgb[y][x][2] = purple.b/2   ; // blue
      //             }
      //          }
      //       }
      //       else
      ...
      //             else
      //             {
      //                rgb[y][x][0] = purple.r * factor   ; // red
      //                rgb[y][x][1] = purple.g * factor   ; // green
      //                rgb[y][x][2] = purple.b * factor   ; // blue
      //             }
      //          }
      //       }
      //    }
      ...
      //    {
      //       rgb[y][x][0] = sky.r   ; // red
      //       rgb[y][x][1] = sky.g   ; // green
      //       rgb[y][x][2] = sky.b   ; // blue
      //    }
      }
   }
   //
